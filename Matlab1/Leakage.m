...
fa>=fo  % sim = 1 e não = 0
% fp=0:df:(Np/2);
fp=[0:df:df*(0.5*Np)];  % vetor de frequencias positivas
%fp=[0:df:df*(0.5*Np-1)];  % vetor de frequencias positivas (no. de ponto par)
fn=[-df*(0.5*Np):df:-df]; % vetor de frequencias negativas
%fn=[-df*(0.5*Np-1):df:0]; % vetor de frequencias negativas (no. de ponto par)
...
% ylabel('|X(f)|')
% title('FFT do sinal c/ vazamento')
% grid
% 
...
Xp=X(1:Np/2+1);  % só a parte positiva da transformada
%Xp=X(1:Np/2);  % só a parte positiva da transformada (no. de ponto par)
figure;
stem(fp,abs(Xp))
xlabel('f')
ylabel('|X(f)|')
